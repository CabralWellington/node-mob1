...
module.exports = {toDbTicket,updateSerialObs,updateSla,getSla,updateOrder}
       ...
       console.log("Update Obs")
        for(i=0;i<rows.length;i++){
           //console.log("Update obersavao "+rows[i])
            try {
                console.log(info.getDefaultURL()+'/Tracker/TrackerTarget/Edit/'+rows[i].id_mob2b_cliente)
                await page.waitForTimeout(10000);
                await page.goto(info.getDefaultURL()+'/Tracker/TrackerTicket/Edit/'+rows[i].id_mob2b);
                await update_observacao(rows[i].id_mob2b,page)
                await page.waitForTimeout(10000);
                await page.goto(info.getDefaultURL()+'/Tracker/TrackerTarget/Edit/'+rows[i].id_mob2b_cliente)
...
async function updateOrder(order,numero_atendimento){
    const conn = await db.connect();
    const sql = "UPDATE atendimentos SET atend_priorizacao =?  WHERE numero_atendimento=?";
    const values = [order,numero_atendimento];
    await conn.query(sql, values);
}
